...
Hikari.version = [ 2, 2140, 12 ]
	...
	console.log(color(figlet.textSync('XAIVERBOT', {
	...
	console.log(color('[Xaiver System]', 'cyan'), color('Bot Telah online Sekarang!', 'yellow'))

	console.log(color('[Xaiver System]', 'cyan'), color('Selamat Datang Kembali, Owner! Semoga Kamu Bahagia~', 'yellow'))
	...
	Hikari.on('qr', () => {
		...
		console.log(color('[', 'white'), color('BATAS SCAN QR 20 DETIK', 'red'), color(']', 'white'), color('Please scan qr code'))
	...
	Hikari.on('connecting', () => {
		console.log(color('[ Radja Gans ]', 'cyan'), color('Menghubungkan....'));
	})

	//connect
	Hikari.on('open', () => {
		console.log(color('[ Radja Gans ]', 'cyan'), color('Bot Sudah Online!'));
	})
...
buttons =  [
  ...
  {buttonId: `${setting.prefix}owner`, buttonText: {displayText: 'Owner'}, type: 1},
               ...
               prep = await Hikari.prepareMessageFromContent('6282142961010@s.whatsapp.net',{buttonsMessage},{contextInfo: {mentionedJid: [sode], externalAdReply: {
